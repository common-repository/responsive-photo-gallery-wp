<?php
	...
	*/
	 ...
	 */
	function basic_plugin_load_textdomain() {
	 ...
	 */
	function tp_photo_gallery_scripts() {
	...
	add_action('wp_enqueue_scripts', 'tp_photo_gallery_scripts');


	/**
	 * Load admin styles and scripts for TP Custom Photo Gallery.
	 */
	function tps_photo_gallery_enqueue_admin_scripts( $hook ) {
	    global $typenow;

	    // Only load scripts and styles for the 'generategallery' post type.
	    if ( $typenow == 'generategallery' ) {
	        // Enqueue admin-specific styles
	        wp_enqueue_style('gallery-admin-style', TP_CUSTOM_PHOTO_GALLERY_PLUGIN_PATH . 'admin/css/tps-photo-gallery-admin.css');
	        
	        // Enqueue admin-specific scripts
	        wp_enqueue_script('gallery-admin-scripts', TP_CUSTOM_PHOTO_GALLERY_PLUGIN_PATH . 'admin/js/tps-photo-gallery-admin-scripts.js', array('jquery'), '1.0.0', true);

	        // Enqueue WordPress color picker
	        wp_enqueue_style('wp-color-picker');
	        wp_enqueue_script('gallery-color-picker', TP_CUSTOM_PHOTO_GALLERY_PLUGIN_PATH . 'admin/js/color-picker.js', array('wp-color-picker'), false, true);
	    }

	    // For 'tp_photo_gallery' post type, load additional scripts
	    if ( $typenow == 'tp_photo_gallery' ) {
	        wp_enqueue_script('gallery-main-admin-scripts', TP_CUSTOM_PHOTO_GALLERY_PLUGIN_PATH . 'admin/js/tps-gallery-main-admin-scripts.js', array('jquery'), '1.0.0', true);
	    }
	}
	add_action('admin_enqueue_scripts', 'tps_photo_gallery_enqueue_admin_scripts');
